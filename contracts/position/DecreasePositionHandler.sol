...
contract DecreasePositionHandler is ReentrancyGuard, IDecreasePositionHandler {
    ...
    function liquidatePosition(PositionLib.LiquidatePositionCache memory cache) external override nonReentrant onlyHandler {
        ...
        if (!cache.isLong) {
            ...
            VaultLib.decreaseGlobalShortSize(dataBase, cache.poolToken, cache.collateralToken, position.size);
        }
    ...
    function _decreasePosition(PositionLib.DecreasePositionCache memory cache) private returns (uint256) {
        ...
        if (!cache.isLong) {
            ...
            VaultLib.decreaseGlobalShortSize(dataBase, cache.poolToken, cache.collateralToken, cache.sizeDelta);
        }
